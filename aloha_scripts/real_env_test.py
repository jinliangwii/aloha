...
def get_action_value(t):
    period = int(1/DT) * 4  # 1000 timesteps to complete one full cycle (adjust as needed)
...
def get_action_value_joint1(t):
    period = int(1/DT) * 8 # 1000 timesteps to complete one full cycle (adjust as needed)
    half_period = period / 2
    value = 0
    if t % period < half_period:
        # Ramp up from 0 to 0.5 over the first half period
        value = 55 * (t % half_period) / half_period
    else:
        # Ramp down from 0.5 to 0 over the second half period
        value = 55 * (1 - (t % half_period) / half_period)

    return value

def get_action_value_joint3(t):
    period = int(1/DT) * 8 # 1000 timesteps to complete one full cycle (adjust as needed)
    half_period = period / 2
    value = 0
    if t % period < half_period:
        # Ramp up from 0 to 0.5 over the first half period
        value = 160 * (t % half_period) / half_period
    else:
        # Ramp down from 0.5 to 0 over the second half period
        value = 160 * (1 - (t % half_period) / half_period)

    return value

def generate_sequence(YT, DT):
    total_steps = 111  # Total steps from -55 to 55 and back to -55
    time_per_step = YT / total_steps
    
    current_value = -55
    direction = 1  # 1 means increasing, -1 means decreasing
    
    start_time = time.time()
    while time.time() - start_time < YT:
        # Print or use current_value here
        print(current_value)
...
def test_real_teleop():
    ...
    totalDuration = int(1/DT) * 8 # in seconds 
    for t in range( totalDuration ):
        # action = get_action(master_bot_left, master_bot_right)

        action_value = get_action_value(t)
        action = [action_value, get_action_value_joint1(t), get_action_value_joint1(t), 0, 0, 0, 0, 0]
        print(action)
